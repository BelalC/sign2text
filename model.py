...
def create_model(model_name, model_weights_path=None, top_model=True, color_mode="rgb", input_shape=None):
    ...
    Returns
    -------
    my_model: tensorflow.keras.model
    ...
    print("[INFO] loading %s..." % (model_name,))
    ...
    if top_model:
        ...
        if model_weights_path is not None:
            ...
            classifier.load_weights(vgg_weights_path)
            ...
            if os.path.exists(res_weights_path):
                ...
                print("[WARNING] ResNet50 pre-trained weights not found at %s" % res_weights_path)
        elif model_name == "mobilenet":
            # pre-trained weights for transfer learning with MobileNet
            if os.path.exists(mob_weights_path):
                classifier.load_weights(mob_weights_path)
            else:
                print("[WARNING] MobileNet pre-trained weights not found at %s" % mob_weights_path)

        # Join pre-loaded model + classification block
        print("[INFO] creating model.")
        my_model = Model(inputs=base_model.input,
                         outputs=classifier(base_model.output))
        return my_model
    else:
        return base_model
